...
class Device:
    def __init__(self, serial: str, logger: Logger, database: WorkerDatabase, notif: NotificationSender, bootloader_timeout_duration: int=30):
        self.serial = serial
        self.logger = logger
        self.dev_files = {}
    ...
    def trackDevice(self, udevinfo: pyudev.Device):
        """Adds a device to the tracked device files."""
    ...
    def handleAddDevice(self, udevinfo: pyudev.Device, have_lock: bool=False):
        """Called by the DeviceManager when a device under this serial is added."""
    ...
    def handleNormalAdd(self, udevinfo: pyudev.Device):
    ...
    def handleRemoveDevice(self, udevinfo: pyudev.Device):
        """Called by the DeviceManager when a device under this serial is removed.
        Removes the device from the tracked dev files."""
        identifier = udevinfo.get("DEVNAME")
    ...
    def handleBootloaderMode(self, udevinfo: pyudev.Device):
    ...
    def handleBootloaderTimeout(self):
        """Callback for when the default firmware flashing process times out. Sets the device to broken."""
        with self.lock:
            if self.mode != Mode.BOOTLOADER:
                return
    ...
    def handleTestAdd(self, udevinfo: pyudev.Device):
